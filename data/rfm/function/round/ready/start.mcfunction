...
execute if score #round round matches 0 run tellraw @a[tag=rfm_participant] {"text":"所有玩家已准备完成，第一回合开始！","color":"green","bold":true}
execute if score #round round matches 1.. run tellraw @a[tag=rfm_participant] {"text":"所有玩家已准备完成，下一回合开始！","color":"green","bold":true}
