...
execute if score @s speech_answer = @s speech_category run tellraw @a[tag=rfm_participant] {"text":"应对成功，演讲顺利继续！","color":"green","bold":true}
...
execute unless score @s speech_answer = @s speech_category run tellraw @a[tag=rfm_participant] {"text":"应对方式错误，现场支持度 -1。","color":"red","bold":true}
