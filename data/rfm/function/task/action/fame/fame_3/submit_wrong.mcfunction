...
tellraw @s [{"text":"本次照片有 ","color":"red"},{"score":{"name":"@s","objective":"photo_match"},"color":"yellow","bold":true},{"text":" / 5 个位置正确。","color":"red"}]
execute if score @s photo_attempt matches 1 run tellraw @s {"text":"你还有一次提交机会，请重新选择居民并调整站位。","color":"aqua"}
