...
execute if score @s audit_answer = @s audit_target run tellraw @a[tag=rfm_participant] {"text":"提交正确，账本中的申报金额与实际核算结果不符！","color":"green","bold":true}
...
execute unless score @s audit_answer = @s audit_target run tellraw @a[tag=rfm_participant] {"text":"提交错误，这本账的申报金额可以由有效支出正确计算得到。","color":"red","bold":true}
