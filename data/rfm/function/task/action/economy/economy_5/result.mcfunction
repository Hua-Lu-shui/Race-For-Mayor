...
execute if score @s employment_completed matches 3..4 run scoreboard players add @s economy 12
execute if score @s employment_completed matches 3..4 run tellraw @a [{"text":"【就业走访】","color":"yellow","bold":true},{"selector":"@s","color":"white"},{"text":" 任务完成！","color":"green","bold":true},{"text":" 经济 +12","color":"yellow"}]

execute if score @s employment_completed matches ..2 run tellraw @a [{"text":"【就业走访】","color":"yellow","bold":true},{"selector":"@s","color":"white"},{"text":" 任务失败。","color":"red","bold":true},{"text":" 属性保持不变","color":"gray"}]
