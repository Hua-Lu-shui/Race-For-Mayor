...
execute if score @s energy_total = @s energy_target run tellraw @a[tag=rfm_participant] [{"text":"供电配平正确！","color":"green","bold":true},{"text":" 当前正确数：","color":"white"},{"score":{"name":"@s","objective":"energy_correct"},"color":"green"}]
...
execute unless score @s energy_total = @s energy_target run tellraw @a[tag=rfm_participant] {"text":"供电配平错误！","color":"red","bold":true}
