...
execute if score @s trash_answer = @s trash_target run tellraw @a[tag=rfm_participant] [{"text":"分类正确！","color":"green","bold":true},{"text":" 当前正确数：","color":"white"},{"score":{"name":"@s","objective":"trash_correct"},"color":"green"}]
execute if score @s trash_answer = @s trash_target run playsound minecraft:entity.experience_orb.pickup master @s ~ ~ ~ 1 1.2 1
execute unless score @s trash_answer = @s trash_target run tellraw @a[tag=rfm_participant] {"text":"分类错误！","color":"red","bold":true}
