...
execute unless score @s action_attribute_missing matches 1 run tellraw @s [{"text":"[名誉行动]","color":"aqua","clickEvent":{"action":"run_command","value":"/trigger action_attribute_choice set 1"}},{"text":"  需要公开表现、沟通或影响舆论的行动","color":"gray"}]

execute unless score @s action_attribute_missing matches 2 run tellraw @s [{"text":"[经济行动]","color":"yellow","clickEvent":{"action":"run_command","value":"/trigger action_attribute_choice set 2"}},{"text":"  需要跑市场、招商或处理财政资源的行动","color":"gray"}]

execute unless score @s action_attribute_missing matches 3 run tellraw @s [{"text":"[民生行动]","color":"red","clickEvent":{"action":"run_command","value":"/trigger action_attribute_choice set 3"}},{"text":"  需要服务居民、处理公共事务的行动","color":"gray"}]

execute unless score @s action_attribute_missing matches 4 run tellraw @s [{"text":"[生态行动]","color":"green","clickEvent":{"action":"run_command","value":"/trigger action_attribute_choice set 4"}},{"text":"  需要环保巡查、治理或绿化建设的行动","color":"gray"}]
