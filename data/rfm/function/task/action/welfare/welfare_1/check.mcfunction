...
execute if score @s hospital_answer = @s hospital_target run tellraw @a[tag=rfm_participant] [{"text":"分诊正确！","color":"green","bold":true},{"text":" 当前正确数：","color":"white"},{"score":{"name":"@s","objective":"hospital_correct"},"color":"green"}]
...
execute unless score @s hospital_answer = @s hospital_target run tellraw @a[tag=rfm_participant] {"text":"分诊错误！","color":"red","bold":true}
