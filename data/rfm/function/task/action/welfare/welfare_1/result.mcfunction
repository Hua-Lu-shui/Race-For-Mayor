...
execute if score @s hospital_correct matches 6..7 run scoreboard players add @s welfare 12
execute if score @s hospital_correct matches 6..7 run tellraw @a [{"text":"【医院排队疏导】","color":"red","bold":true},{"selector":"@s","color":"white"},{"text":" 任务完成！","color":"green","bold":true},{"text":" 民生 +12","color":"red"}]

execute if score @s hospital_correct matches ..5 run tellraw @a [{"text":"【医院排队疏导】","color":"red","bold":true},{"selector":"@s","color":"white"},{"text":" 任务失败。","color":"red","bold":true},{"text":" 属性保持不变","color":"gray"}]
