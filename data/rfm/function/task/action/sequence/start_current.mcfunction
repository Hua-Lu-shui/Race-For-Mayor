...
#公布当前行动玩家
execute as @a[tag=rfm_participant] if score @s action_order = #current_action action_order if score @s action_role matches 1 run tellraw @a[tag=rfm_participant] [{"text":"现在由","color":"white"},{"text":"行动玩家 ","color":"#67D5FF","bold":true},{"selector":"@s","color":"white"},{"text":" 执行任务。","color":"white"}]
