execute as @a[tag=rfm_participant,scores={group_finished=0},limit=1] run tellraw @a[tag=rfm_participant] [{"selector":"@s","color":"gold","bold":true},{"text":" 成为击鼓传花第一名！","color":"gold","bold":true}]
