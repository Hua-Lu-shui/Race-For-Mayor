...
execute unless entity @a[tag=rfm_participant,tag=!rfm_potato_holder,scores={potato_pos=8,group_finished=0},limit=1] run return 0
tag @a[tag=rfm_participant] remove rfm_hot_potato_target_player
tag @a[tag=rfm_participant,tag=!rfm_potato_holder,scores={potato_pos=8,group_finished=0},limit=1] add rfm_hot_potato_target_player
