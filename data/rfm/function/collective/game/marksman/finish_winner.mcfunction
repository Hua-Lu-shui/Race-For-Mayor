execute as @a[tag=rfm_participant,scores={group_finished=0},limit=1] run tellraw @a[tag=rfm_participant] [{"selector":"@s","color":"gold","bold":true},{"translate":"rfm.collective.marksman.winner","color":"gold","bold":true}]
