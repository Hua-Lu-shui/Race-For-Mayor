#若非名誉属性低于本轮任务开始前的值，则恢复；任务带来的提升予以保留
...
execute if score @s ability_trigger matches 1 run tellraw @s [{"text":"【粉丝效应】","color":"aqua","bold":true},{"text":"每轮结束：已恢复因任务而降低的除名誉外的属性。","color":"white"}]
