...
execute unless entity @a[tag=rfm_participant,scores={room=2}] run return run tellraw @s {"text":"该玩家当前不在线，请选择其他玩家。","color":"red"}
execute if entity @a[tag=rfm_participant,scores={room=2,candidate=7}] run return run tellraw @s {"text":"律师受【维权意识】保护，不能成为属性互换目标。","color":"light_purple"}
